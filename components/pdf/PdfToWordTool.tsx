...
interface WordConversionResult {
  url: string;
  filename: string;
  totalPages: number;
...
function WordDocIcon(props: SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" {...props}>
      <path
        d="M7 4h7l4 4v12H7z"
      ...
      />
      <path
        d="M9.5 10.5 11 16l1.4-3.6L13.8 16l1.5-5.5"
        fill="none"
        stroke="currentColor"
        strokeWidth="1.75"
        strokeLinecap="round"
...
export function PdfToWordTool() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        let errText = "Failed to convert PDF to Word.";
        try {
      ...
      const extractedWords = parseInt(
        response.headers.get("X-Extracted-Words") || "0",
        10
      );
      const extractedParagraphs = parseInt(
        response.headers.get("X-Extracted-Paragraphs") || "0",
        10
      );
      ...
      {result ? (
        <div className="relative overflow-hidden rounded-3xl border border-emerald-200 bg-card p-6 shadow-md sm:p-10 text-center">
          ...
          <p className="mx-auto mt-2 max-w-lg text-sm leading-relaxed text-slate sm:text-base">
            Your PDF has been converted into a fully editable Microsoft Word (.docx) document with
            structural paragraphs and headings.
          </p>
          ...
          {result.isScanned && (
            <div className="mx-auto mt-5 max-w-lg rounded-2xl border border-amber-200 bg-amber-50/90 p-4 text-left text-xs text-amber-950 shadow-2xs">
              <p className="font-semibold text-amber-900">ℹ️ Scanned PDF Notice</p>
              <p className="mt-1 leading-relaxed text-amber-800">
                This document contains minimal selectable text. Embedded images were placed into the
                Word document. Full searchable text extraction on scanned documents will be
                available with <strong>Smart OCR</strong>.
              </p>
            </div>
          )}
          ...
          <div className="mx-auto mt-7 grid max-w-lg grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <div className="rounded-2xl border border-line bg-background/80 p-3.5">
              ...
              <span className="mt-1 block text-sm font-bold text-emerald-700 sm:text-base">
                {result.extractedWords.toLocaleString()}
              </span>
            </div>
        ...
        <div className="space-y-6">
          {!pdfMeta ? (
            ...
            <div className="rounded-3xl border border-line/90 bg-card/60 p-5 shadow-xs sm:p-7">
              <FileUpload
                title="Drop a PDF file to convert to Word, or browse"
                description="Upload exactly 1 PDF document (up to 50 MB) to extract editable text, headings, and tables."
              ...
              <div className="rounded-3xl border border-line bg-card p-5 sm:p-8 shadow-xs space-y-6">
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate">
                    FileNova accurately reconstructs document layout into clean OpenXML Word elements.
                  </p>
                </div>

                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div className="flex items-start gap-3 rounded-2xl border border-line bg-background/60 p-4">
                    <span className="text-xl">📝</span>
                    <div>
                      <h4 className="text-xs font-semibold text-ink">Paragraphs &amp; Headings</h4>
                      <p className="mt-0.5 text-xs text-slate">
                        Detects title and section font sizes to create genuine H1, H2, and body
                        paragraphs.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3 rounded-2xl border border-line bg-background/60 p-4">
                    <span className="text-xl">📊</span>
                    <div>
                      <h4 className="text-xs font-semibold text-ink">Editable Word Tables</h4>
                      <p className="mt-0.5 text-xs text-slate">
                        Identifies multi-column aligned data and outputs standard styled Word tables.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3 rounded-2xl border border-line bg-background/60 p-4">
                    ...
                    <div>
                      <h4 className="text-xs font-semibold text-ink">Embedded Images</h4>
                      <p className="mt-0.5 text-xs text-slate">
                        Extracts and places JPEG photos directly into document sections.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3 rounded-2xl border border-line bg-background/60 p-4">
                    <span className="text-xl">🔒</span>
                    <div>
                      <h4 className="text-xs font-semibold text-ink">Privacy Preserving</h4>
                      <p className="mt-0.5 text-xs text-slate">
                        Converted strictly in-memory. Never written to permanent disk storage.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 rounded-2xl border border-line/80 bg-background/70 p-4 text-xs text-slate">
                  <WordDocIcon className="size-5 shrink-0 text-ember" />
                  <p className="leading-relaxed">
                    Ready to convert <strong>{pdfMeta.totalPages} {pdfMeta.totalPages === 1 ? "page" : "pages"}</strong> into a native <strong>.docx</strong> file readable by Microsoft Word, Google Docs, and LibreOffice.
                  </p>
                </div>
